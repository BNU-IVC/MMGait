...
class fusion_imgimg(BaseModel):

    def build_network(self, model_cfg):
        B, C = model_cfg['Backbone']['blocks'], model_cfg['Backbone']['C']
    ...
    def forward(self, inputs):
        ...
        part1 = self.img_pretreat(ipts[0])
        part2 = self.img_pretreat(ipts[1])
        ...
        if N1 == N2:
            ...
            if N1 < target_len:
                repeat_factor = target_len // N1
                remainder = target_len % N1
                part1 = part1.repeat(1, 1, repeat_factor, 1, 1)
                if remainder > 0:
                    part1 = torch.cat([part1, part1[:, :, :remainder]], dim=2)
            elif N2 < target_len:
                repeat_factor = target_len // N2
                remainder = target_len % N2
                part2 = part2.repeat(1, 1, repeat_factor, 1, 1)
                if remainder > 0:
                    part2 = torch.cat([part2, part2[:, :, :remainder]], dim=2)
        del ipts
