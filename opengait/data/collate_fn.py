...
class CollateFn(object):
    ...
    def __call__(self, batch):
        ...
        seqs_batch, labs_batch, typs_batch, vies_batch, modal_valid_batch = [], [], [], [], []
        ...
        for bt in batch:
            ...
            modal_valid_batch.append(bt[1][3])
        ...
        def sample_frames(seqs):
            ...
            for i in range(feature_num):
                ...
                for j in indices[:self.frames_all_limit] if self.frames_all_limit > -1 and len(indices) > self.frames_all_limit else indices:
                    ...
                    if self.points_in_use is not None and point_cloud_index is not None and i in point_cloud_index:
        ...
        batch = [fras_batch, labs_batch, typs_batch, vies_batch, modal_valid_batch, None]
        ...
        if self.sampler == "fixed":
            ...
            seqL_batch = [[len(fras_batch[i][j]) for i in range(batch_size)]
                          for j in range(feature_num)]  # [f, b]
