...
def single_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    probe_seq_dict = {'CASIA-B': {'NM': ['nm-05', 'nm-06'], 'BG': ['bg-01', 'bg-02'], 'CL': ['cl-01', 'cl-02']},
                      ...
                      'MMGait': {'NM': ['NM-02'], 'BG': ['BG-01'], 'CL': ['CL-01']},  
                      }
...
def evaluate_indoor_dataset(data, dataset, metric='euc', cross_view_gallery=False):
    ...
    if dataset not in ('CASIA-B', 'OUMVLP', 'CASIA-E', 'SUSTech1K', 'MMGait'):
...
def mmgait_single_modal_evaluation(data, dataset, metric='euc'):
    ...
    for (type_, probe_seq) in probe_seq_dict.items():
        ...
        for (v1, probe_view) in enumerate(view_list):
            ...
            for (v2, gallery_view) in enumerate(view_list):
                gseq_mask = np.isin(seq_type, gallery_seq_dict) & np.isin(
                    view, [gallery_view])
...
def mmgait_cross_modal_evaluation(data, dataset, metric):
    feature, label, seq_type, view = data['embeddings'], data['labels'], data['types'], data['views']
    label = np.array(label)
    view = np.array(view)
    msg_mgr = get_msg_mgr()
    ...
    feature_probe, feature_gallery = feature[:,:,:feature.shape[-1]//2],feature[:,:,feature.shape[-1]//2:]
    acc = {}
    mean_ap = {}
    view_list = sorted(np.unique(view))
    num_rank = 1
    ...
    for (type_, probe_seq) in probe_seq_dict.items():
        ...
        for (v1, probe_view) in enumerate(view_list):
            pseq_mask = np.isin(seq_type, probe_seq) & np.isin(
                view, probe_view)
            probe_x = feature_probe[pseq_mask, :]
            probe_y = label[pseq_mask]
            ...
            for (v2, gallery_view) in enumerate(view_list):
                gseq_mask = np.isin(seq_type, gallery_seq_dict) & np.isin(
                    view, [gallery_view])
                gallery_y = label[gseq_mask]
                gallery_x = feature_gallery[gseq_mask, :]
                dist = cuda_dist(probe_x, gallery_x, metric)
                idx = dist.topk(num_rank, largest=False)[1].cpu().numpy()
                acc[type_][v1, v2, :] = np.round(np.sum(np.cumsum(np.reshape(probe_y, [-1, 1]) == gallery_y[idx[:, 0:num_rank]], 1) > 0,
                                                     0) * 100 / dist.shape[0], 2)
    ...
    result_dict = {}
    msg_mgr.log_info('===Rank-1 (Exclude identical-view cases, Modal1 -> Modal2)===')
    out_str = ""
    out_map_str = "============= mAP ============\n"
    for rank in range(num_rank):
        out_str = ""
    ...
    label = np.array(label)
    view = np.array(view)
    feature_gallery, feature_probe = feature[:,:,:feature.shape[-1]//2],feature[:,:,feature.shape[-1]//2:]
    acc = {}
    mean_ap = {}
    view_list = sorted(np.unique(view))
    num_rank = 1
    ...
    result_dict = {}
    msg_mgr.log_info('===Rank-1 (Exclude identical-view cases, Modal2 -> Modal1)===')
    out_str = ""
    out_map_str = "============= mAP ============\n"
    for rank in range(num_rank):
        out_str = ""
...
def mmgait_cal_metric(feature_probe, feature_gallery, label, seq_type, view, msg_mgr, probe_name, gallery_name,metric):
    ...
    result_dict = {}
    msg_mgr.log_info('===Rank-1 (Exclude identical-view cases, {} -> {})==='.format(probe_name, gallery_name))
    out_str = ""
    out_map_str = "============= mAP ============\n"
    for rank in range(num_rank):
        out_str = ""
...
def mmgait_omni_multi_modal_evaluation(data, dataset, metric='euc'):
    ...
    depth_embed, event_embed, heatmap_embed, ir_embed, ir_sils_embed, lidar_depth_embed, radar_depth_embed, rgb_embed, rgb_sils_embed = feature

    modal_embeddings = {
        "depth": depth_embed,
        "event": event_embed,
        "heatmap": heatmap_embed,
        "ir": ir_embed,
        "ir_sils": ir_sils_embed,
        "lidar_depth": lidar_depth_embed,
        "radar_depth": radar_depth_embed,
        "rgb": rgb_embed,
